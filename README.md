...
multiplied by that ball's combo. Each ball starts at `x1`, increases to `x2` after its
first block hit, and resets to `x1` when it touches a stage boundary. Only multipliers
above `x1` are displayed. Clearing every block opens the shop, where credits
